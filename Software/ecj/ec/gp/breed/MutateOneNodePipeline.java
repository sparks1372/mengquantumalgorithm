...
/**
 * MutateOneNodesPipeline implements the OneNode mutation algorithm described in
 * Kumar Chellapilla, "A Preliminary Investigation into Evolving Modular
 * Programs without Subtree Crossover", GP98.
 * 
 * <p>
 * MutateOneNodesPipeline chooses a single node in an individual and replaces it
 * with a randomly-chosen node of the same arity and type constraints. Thus the
 * original topological structure is the same but that one node is different.
 * 
 * 
 * <p>
 ...
 * <table>
 * <tr>
 * <td valign=top><i>base</i>.<tt>ns</tt>.0<br>
 ...
 * <td>The GPNodeSelector selector</td>
 * </tr>
 * </table>
 * 
 * @author Sean Luke
	...
	public int produce(final int min, final int max, final int start,
		...
		for (int q = start; q < n + start; q++) {
			...
			if ((tree != TREE_UNFIXED)
				...
				// uh oh
				state.output
						.fatal("MutateOneNodePipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual");
			}
