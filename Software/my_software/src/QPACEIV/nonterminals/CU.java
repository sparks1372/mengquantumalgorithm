...
public class CU extends NonTerminalSuper {
	...
	public void eval(final EvolutionState state, final int thread,
		...
		for (int i = 0; i < Math.abs(targ - ctrl) - 1; i++) {
		...
		if (Math.min(targ, ctrl) != 0) {
			operation = Tensor_Matrix.tensor_prod(Matrix.identity(
					(int) Math.pow(2, ctrl < targ ? ctrl : targ),
					(int) Math.pow(2, ctrl < targ ? ctrl : targ)), operation);
		}

		if (Math.max(targ, ctrl) != rd.NUM_OF_QUBITS - 1) {
			operation = Tensor_Matrix.tensor_prod(operation, Matrix.identity(
					(int) Math.pow(2, ctrl > targ ? (rd.NUM_OF_QUBITS - 1)
							- ctrl : (rd.NUM_OF_QUBITS - 1) - targ),
					(int) Math.pow(2, ctrl > targ ? (rd.NUM_OF_QUBITS - 1)
							- ctrl : (rd.NUM_OF_QUBITS - 1) - targ)));
		}
		// operation.print(0, 0);
