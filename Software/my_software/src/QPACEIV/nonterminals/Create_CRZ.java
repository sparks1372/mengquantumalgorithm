...
public class Create_CRZ extends GPNode {
	...
	public String toString() {
		return "Create_CRZ(" + children[1].toString() + ")";
	}
		...
		for (int index = 0; index < Qprob.NUM_OF_QUBITS; index++) {
			if (index == ctrl) {
				...
				Qprob.circuit[index].add(new CU(targ, ctrl,
						new RZ(children[2]), (QuantumProblem) problem));
			} else {
	...
	public static void main(String[] args) {
		Create_CRZ test_create_crz = new Create_CRZ();
		QuantumProblem qprob = new QuantumProblem(2);

		test_create_crz.children[0] = new Qubit(0);
		test_create_crz.children[1] = new Qubit(1);
		test_create_crz.children[2] = new piovfourrad();

		test_create_crz.eval(null, 0, new QPaceIVQuantumState(), null, null,
				qprob);
