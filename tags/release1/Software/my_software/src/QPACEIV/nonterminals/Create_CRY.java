...
public class Create_CRY extends GPNode {
	...
	public String toString() {
		return "Create_CRY(" + children[1].toString() + ")";
	}
		...
		for (int index = 0; index < Qprob.NUM_OF_QUBITS; index++) {
			if (index == ctrl) {
				...
				Qprob.circuit[index].add(new CU(targ, ctrl,
						new RY(children[2]), (QuantumProblem) problem));
			} else {
	...
	public static void main(String[] args) {
		Create_CRY test_create_cry = new Create_CRY();
		QuantumProblem qprob = new QuantumProblem(2);

		test_create_cry.children[0] = new Qubit(0);
		test_create_cry.children[1] = new Qubit(1);
		test_create_cry.children[2] = new piovfourrad();

		test_create_cry.eval(null, 0, new QPaceIVQuantumState(), null, null,
				qprob);
