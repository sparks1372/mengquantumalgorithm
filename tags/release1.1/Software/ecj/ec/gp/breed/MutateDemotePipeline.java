 ...
 * described in Kumar Chellapilla, "A Preliminary Investigation into Evolving
 * Modular Programs without Subtree
 * Crossover", GP98, and is also similar to the "insertion" operator found in
 * Una-May O'Reilly's thesis, <a
 * href="http://www.ai.mit.edu/people/unamay/thesis.html">
 * "An Analysis of Genetic Programming"</a>.
 * 
 * <p>
 * MutateDemotePipeline tries picks a random tree, then picks randomly from all
 * the demotable nodes in the tree, and demotes one. If its chosen tree has no
 * demotable nodes, or demoting its chosen demotable node would make the tree
 * too deep, it repeats the choose-tree-then-choose-node process. If after
 * <i>tries</i> times it has failed to find a valid tree and demotable node, it
 * gives up and simply copies the individual.
 * 
 * <p>
 * "Demotion" means to take a node <i>n</i> and insert a new node <i>m</i>
 * between <i>n</i> and <i>n</i>'s parent. <i>n</i> becomes a child of <i>m</i>;
 * the place where it becomes a child is determined at random from all the
 * type-compatible slots of <i>m</i>. The other child slots of <i>m</i> are
 * filled with randomly-generated terminals. Chellapilla's version of the
 * algorithm always places <i>n</i> in child slot 0 of <i>m</i>. Because this
 * would be unneccessarily restrictive on strong typing, MutateDemotePipeline
 * instead picks the slot at random from all available valid choices.
 * 
 * <p>
 * A "Demotable" node means a node which is capable of demotion given the
 * existing function set. In general to demote a node <i>foo</i>, there must
 * exist in the function set a nonterminal whose return type is type-compatible
 * with the child slot <i>foo</i> holds in its parent; this nonterminal must
 * also have a child slot which is type-compatible with <i>foo</i>'s return
 * type.
 * 
 * <p>
 * This method is very expensive in searching nodes for "demotability". However,
 * if the number of types is 1 (the GP run is typeless) then the
 * type-constraint-checking code is bypassed and the method runs a little
 * faster.
 * 
 * 
 * <p>
 ...
 * <font size=-1>int &gt;= 1</font></td>
 * <td valign=top>(maximum valid depth of a mutated tree)</td>
 * </tr>
 * 
 * <tr>
...
public class MutateDemotePipeline extends GPBreedingPipeline {
	...
	private void _demoteSomething(final GPNode node,
		...
		for (int x = 0; x < set.nonterminals[t.type].length; x++) {
			for (GPType childtype : set.nonterminals[t.type][x]
					.constraints(initializer).childtypes) {
				if (childtype.compatibleWith(initializer,
						node.constraints(initializer).returntype)) {
					numDemotable++;
					break; // breaks out to enclosing for
				}
			}
		}
			...
			for (GPType childtype : set.nonterminals[t.type][x]
				...
				if (childtype.compatibleWith(initializer,
					...
					if (numDemotable == demoteItem) {
						// clone the node
						GPNode cnode = (set.nonterminals[t.type][x]
								.lightClone());
						...
						for (int z = 0; z < cnode.children.length; z++) {
							if (chityp[z].compatibleWith(initializer, retyp)) {
								if (numSpots == choice) {
									...
									// hang a randomly-generated terminal off of
									// cnode
									GPNode term = (set.terminals[chityp[z].type][state.random[thread]
											.nextInt(set.terminals[chityp[z].type].length)]
											.lightClone());
									cnode.children[z] = term;
									term.parent = cnode; // just in case
									term.argposition = (byte) z; // just in case
									term.resetNode(state, thread); // let it
																	// randomize
																	// itself if
																	// necessary
							...
							} else if (set.terminals[chityp[z].type].length != 0) {
								// hang a randomly-generated terminal off of
								// cnode
								GPNode term = (set.terminals[chityp[z].type][state.random[thread]
										.nextInt(set.terminals[chityp[z].type].length)]
	...
	private void _demoteSomethingTypeless(final GPNode node,
			final EvolutionState state, final int thread,
			final GPFunctionSet set) {
		int numDemotable = 0;
		...
		GPType[] chityp = cnode
				.constraints(((GPInitializer) state.initializer)).childtypes;
		...
		for (int z = 0; z < cnode.children.length; z++) {
			if (z == choice) {
				...
				term.parent = cnode; // just in case
				term.argposition = (byte) z; // just in case
				term.resetNode(state, thread); // let it randomize itself if
												// necessary
			}
		}
	}
	...
	private int _numDemotableNodes(final GPInitializer initializer,
		...
		for (GPNode element : root.children) {
			soFar = _numDemotableNodes(initializer, element, soFar, set);
		}
		return soFar;
	}
	...
	private int _pickDemotableNode(final GPInitializer initializer,
		...
		for (GPNode element : root.children) {
			num = _pickDemotableNode(initializer, element, num, set);
			if (num == -1) {
	...
	private void demoteSomething(final GPNode node, final EvolutionState state,
			final int thread, final GPFunctionSet set) {
		...
		if (((GPInitializer) state.initializer).numAtomicTypes
			...
			_demoteSomething(node, state, thread, set);
		}
	}
	...
	private int numDemotableNodes(final GPInitializer initializer,
		...
		if (initializer.numAtomicTypes + initializer.numSetTypes == 1) {
			...
			return _numDemotableNodes(initializer, root, soFar, set);
		}
	}
	...
	private int pickDemotableNode(final GPInitializer initializer,
		...
		if (initializer.numAtomicTypes + initializer.numSetTypes == 1) {
			...
			if (gatherer.node == null) {
				throw new InternalError(
						"Internal error in pickDemotableNode, nodeInPosition didn't find a node!"); // should
																									// never
																									// happen
			}
			...
			return _pickDemotableNode(initializer, root, num, set);
		}
	}
	...
	public int produce(final int min, final int max, final int start,
		...
		for (int q = start; q < n + start; q++) {
			...
			if ((tree != TREE_UNFIXED)
				...
				// uh oh
				state.output
						.fatal("MutateDemotePipeline attempted to fix tree.0 to a value which was out of bounds of the array of the individual's trees.  Check the pipeline's fixed tree values -- they may be negative or greater than the number of trees in an individual");
			}
			...
			for (int x = 0; x < numTries; x++) {
				...
				int numdemote = numDemotableNodes(initializer,
						j.trees[t].child, 0,
				...
				// demote the node, or if we're unsuccessful, just leave it
				// alone
				pickDemotableNode(initializer, j.trees[t].child,
						state.random[thread].nextInt(numdemote),
						j.trees[t].constraints(initializer).functionset);

				// does this node exceed the maximum depth limits?
				if (!verifyPoint(demotableNode)) {
					continue; // uh oh, try again
				}

				// demote it
				demoteSomething(demotableNode, state, thread,
						j.trees[t].constraints(initializer).functionset);
				j.evaluated = false;
				break;
			}
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		if (numTries == 0) {
			state.output
					.fatal("MutateDemotePipeline has an invalid number of tries (it must be >= 1).",
		...
		if (maxDepth == 0) {
			state.output.fatal("The MutateDemotePipeline " + base
