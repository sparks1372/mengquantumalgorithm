...
public class Create_Rz extends GPNode {
	...
	public String toString() {
		return "Create_RZ(" + children[1].toString() + ")";
	}
	...
	public void eval(EvolutionState state, int thread, GPData input,
		...
		for (int index = 0; index < Qprob.NUM_OF_QUBITS; index++) {
			if (index == rd.in) {
				Qprob.circuit[index].add(new RZ(children[1]));
			} else {
	...
	public static void main(String[] args) {
		Create_Rz test_create_rz = new Create_Rz();
		QuantumProblem qprob = new QuantumProblem(2);

		test_create_rz.children[0] = new Qubit(0);
		test_create_rz.children[1] = new piovfourrad();

		test_create_rz.eval(null, 0, new QPaceIVQuantumState(), null, null,
				qprob);
