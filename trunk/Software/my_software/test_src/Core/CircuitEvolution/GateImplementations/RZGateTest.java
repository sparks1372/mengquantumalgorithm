...
public class RZGateTest {
	...
	public void testApply() {
		...
		test = new RZ_Gate(1, theta);
		test_state = predefined_states.get_1q_0();

		state_array = new Complex[2][1];
		state_array[0][0] = new Complex(Math.cos(-theta / 2),
				Math.sin(-theta / 2));
		...
		state_array[0][0] = new Complex(0, 0);
		state_array[1][0] = new Complex(0, 0);
		state_array[2][0] = new Complex(Math.cos(theta / 2),
				Math.sin(theta / 2));
		state_array[3][0] = new Complex(0, 0);
