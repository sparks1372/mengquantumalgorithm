	...
	public synchronized void update(Observable arg0, Object arg1) {
		if (arg0 instanceof qcevolutionbackend) {
			...
			if ((backend.getCurrentse().getState() == SearchEngineState.SearchCompleteResultAvailable)
					&& (backend.getCurrentse().getResults().length > 0)) {
				SearchResult[] results = backend.getCurrentse().getResults();
				...
				resultCollectionPanel.setVisible(true);
			} else if (backend.getCurrentse().getState() == SearchEngineState.Searching) {
				selectAndStatPanel.setVisible(false);
				resultCollectionPanel.setVisible(false);
			}
		}
