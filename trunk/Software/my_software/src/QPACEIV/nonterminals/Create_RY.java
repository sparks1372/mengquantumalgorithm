...
public class Create_RY extends GPNode {
	...
	public String toString() {
		return "Create_RY(" + children[1].toString() + ")";
	}
	...
	public void eval(EvolutionState state, int thread, GPData input,
		...
		for (int index = 0; index < Qprob.NUM_OF_QUBITS; index++) {
			if (index == rd.in) {
				Qprob.circuit[index].add(new RY(children[1]));
			} else {
	...
	public static void main(String[] args) {
		Create_RY test_create_ry = new Create_RY();
		QuantumProblem qprob = new QuantumProblem(2);

		test_create_ry.children[0] = new Qubit(0);
		test_create_ry.children[1] = new piovfourrad();

		test_create_ry.eval(null, 0, new QPaceIVQuantumState(), null, null,
				qprob);
