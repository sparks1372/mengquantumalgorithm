...
public class Create_CP extends GPNode {
	...
	public String toString() {
		return "Create_CP(" + children[1].toString() + ")";
	}
	...
	public void eval(EvolutionState state, int thread, GPData input,
		...
		for (int index = 0; index < Qprob.NUM_OF_QUBITS; index++) {
			if (index == ctrl) {
				...
				Qprob.circuit[index].add(new CU(targ, ctrl, new Phase(
						children[2]), (QuantumProblem) problem));
			} else {
	...
	public static void main(String[] args) {
		Create_CP test_create_cp = new Create_CP();
		QuantumProblem qprob = new QuantumProblem(2);

		test_create_cp.children[0] = new Qubit(0);
		test_create_cp.children[1] = new Qubit(1);
		test_create_cp.children[2] = new piovfourrad();

		test_create_cp.eval(null, 0, new QPaceIVQuantumState(), null, null,
				qprob);
