...
It also ensure that researchers can distribute Test Suite easier than if all Search Problems were fully specified in a single file.

\begin{figure}
\centering
\begin{emp}[classdiag](20, 20)
Class.A("TestCase")("-StartState:Matrix","-FinalState:Matrix","-Label:String","-Id:int")();
Class.B("TestSet")("-NumberOfQubits:int")();
Class.C("TestSuite")()();
...
It can be seen in the class diagram, in Figure \ref{fig:testsuiteclassdiag}, that an additional class, SuperPositionalTestSet, is included as a subclass of the TestSet class.
In the framework the test cases that have starting states in a superposition are held within SuperPositionalTestSets.
A SuperPositionalTestSet for 2 qubits returns ``-2'' to indicate the superposition starting state.
This initially doesn't seem that useful as both a superpositional and non-superpositional test case contain both a start and final state that are treated exactly the same during evaluation.
This is however useful for GUI's and 3rd party software and also for the creation of test cases from the XML files.
The labelling of non-superpositional test cases, to make most sense to users, are set to the binary value of the input state.
For example, the label for the input state $\begin{pmatrix}1\\0\end{pmatrix}$ is set as $0$ and for the input state $\begin{pmatrix}0\\1\end{pmatrix}$ the label is set as $1$.
However, for superpositional test cases the use of the expanded superpositional state, $\begin{pmatrix}\frac{1}{\sqrt{2}}\\\frac{1}{\sqrt{2}}\end{pmatrix}$ to $\frac{1}{\sqrt{2}}\ket{0}+\frac{1}{\sqrt{2}}\ket{1}$, is not sensible due to the exponential increase in size as the number of qubits increases, therefore reducing readability.
For the superpositional test cases the labels are therefore set as ``Test Case $0$'' to ``Test Case $n-1$''.

The negation of the ``NumQubits'' attribute is used to specify in the XML file that the test set is of superpositional input states.
This negation is required for the labels of the test cases to be set correctly.

Alongside the framework, an independant test suite graphical editor is provided.
A user can use this to produce the XML file and therefore do not need to explicitly consider the XML file.
